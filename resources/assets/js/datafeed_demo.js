Datafeeds = {}, Datafeeds.UDFCompatibleDatafeed = function () {
				...
				return $.each(c.submarkets, function (b, g) {
					return $.each(g.symbols, function (b, g) {
						if (g.symbol.indexOf(a) !== -1) {
							...
							var h = d._symbolMetadata(c.name),
								i = h.pricescale,
								j = h.minmov,
								k = h.session;

							console.log(h.pricescale);
							console.log(h.minmov);
	...
	return this.ws = new WebSocket("wss://frontend.binaryws.com/websockets/v3?l=en&app_id=2742"), this.ws.onopen = function (b) {
		...
		a.globalNotifier.noConnectionNotification(), $.growl.error({
			message: "Connection error. Refresh page!"
		}), a.unresolved_promises = [], a.callbacks = []
	...
	var b = this,
		c = a.req_id && Object.keys(a).length > 1;
	return c ? new Promise(function (c, d) {
		b.unresolved_promises[a.req_id] = {
			resolve: c,
			reject: d
	...
	var b = a.count || 5e3,
		c = a.granularity,
		d = null;
	if (a.startTime) d = moment.utc(a.startTime);
	else {
		d = moment.utc();
...
var CommonUtils = function () {
		...
		a.prototype.parseSuffixAndIntValue = function () {
			...
			var a = TradingView.actualResolution.toUpperCase().replace("D", "").replace("M", "").replace("W", ""),
				b = "" === a ? 1 : parseInt(a),
				c = TradingView.actualResolution.replace("" + b, "");
			switch (c) {
				case "":
					b < 60 ? c = "M" : (b /= 60, c = "H");
					break;
...
HistoricalOHLCReqRespHandler = function (a) {
		...
		"use strict";
		var d = this._commonUtils.parseSuffixAndIntValue(),
			e = d.suffix,
			f = d.intVal,
			g = this._commonUtils.totalSecondsInABar(e, f);
		this._binary_websockets.request_candles({
			symbol: a.ticker,
			granularity: g
		}).catch(function () {
			c()
		...
		var d = this,
			e = this._commonUtils.parseSuffixAndIntValue(),
		...
		this._streamingMap[c] = {
			...
			timerCallback: function () {
				...
				}).catch(function (a) {}).then(function (c) {
					c && c.candles && c.candles.forEach(function (c) {
						var d = 1e3 * parseInt(c.epoch),
							e = parseFloat(c.open),
							f = parseFloat(c.high),
							g = parseFloat(c.low),
							h = parseFloat(c.close),
							i = {
								time: d,
								open: e,
								high: f,
			...
			streamingCallback: function (a) {
				...
				if (e && a.ohlc.symbol === e.symbol && a.ohlc.granularity === e.granularity) {
					...
					var f = 1e3 * parseInt(a.ohlc.open_time),
						g = parseFloat(a.ohlc.open),
						h = parseFloat(a.ohlc.high),
						i = parseFloat(a.ohlc.low),
						j = parseFloat(a.ohlc.close);
					f && g && h && i && j && (this.lastBar = {
						time: f,
						open: g,
						high: h,
						low: i,
		...
		b.timerHandler ? clearInterval(b.timerHandler) : this._binary_websockets.request_stop_ohlc_streaming(b.server_request_id, a).then(function () {}).catch(function () {}), delete this._streamingMap[a]
		...
		for (var b = 0; b < a.trading_times.markets.length; b++) {
			...
			for (var d = {
					name: c.name,
					submarkets: []
				}, e = 0; e < c.submarkets.length; ++e) {
				for (var f = c.submarkets[e], g = {
						name: f.name,
						symbols: []
					}, h = 0; h < f.symbols.length; h++) {
					var i = f.symbols[h];
	...
	function (a) {
		a.fn.bindFirst = function (b, c, d) {
			...
			return b = e > 0 ? b.substring(0, e) : b, d = void 0 === d ? c : d, c = "function" == typeof c ? {} : c, this.each(function () {
				...
				var h = e.data("events") || a._data(e[0], "events"),
					i = h[b],
					j = i.pop();
				i.unshift(j)
			})
