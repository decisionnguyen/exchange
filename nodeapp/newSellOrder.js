...
socket.on('connect', function () {
	...
	socket.on('new-sell-order', function(data){
		...
		console.log("NEW "+data.type.toUpperCase()+" SELL ORDER : " + hash);
		...
		checkMarketStatus(market_id, function () {
			...
			checkProccessing(data.user_id, function  () {
				...
				checkWallet(data.user_id,data.market.coin_id_first,data.amount, function (err,avaiable) {
...
function deleteOrder (id, market_id, user_id, callback) {
	pool.query("DELETE FROM orders WHERE id=" + id, function () {
...
function updateOrder (id,amount,value,total,market_id, user_id, callback) {
	pool.query("UPDATE orders SET amount="+amount+",value="+value+",total="+total+" WHERE id=" + id, function () {
		// emit all my order in private
		pool.query("SELECT * FROM orders WHERE market_id="+market_id+" AND user_id="+user_id+" ORDER BY id desc LIMIT 100", function(err, res) {
			socket.emit('change-my-order', {channel: 'private-' + user_id, password: SOCKET_PASSWORD, message: {data: res.rows, market_id: market_id}});
			callback();
...
function insertMarketTradeHistory (market_id,price,amount,value,fee,total,market_symbol) {
	pool.query("INSERT INTO market_trade_histories(market_id,type,price,amount,value,fee,total,created_at,updated_at) VALUES ("+market_id+",'sell',"+price+","+amount+","+value+","+fee+","+total+",'"+getNowFormat()+"','"+getNowFormat()+"')", function () {
...
function solveStopLimit (data,index) {
	...
	pool.query("INSERT INTO orders(market_id,user_id,type,price,amount,value,fee,total,created_at,updated_at) VALUES("+arr.market_id+","+arr.user_id+",'sell',"+arr.price+","+arr.amount+","+arr.value+","+arr.fee+","+arr.total+",'"+getNowFormat()+"', '"+getNowFormat()+"')", function (err) {
...
function solve (data, index, order, callback) {
	...
	if(data.length == index) {
		...
		if(order.type == 'limit' && order.amount != 0) {
			var value = parseFloat(order.amount) * parseFloat(order.price);
			var total = value - (value * order.fee / 100);

			subtractWallet(order.user_id,order.market.coin_id_first,order.amount,order.coinFirst.symbol, function (err,balance) {
				if(err) return callback();
				emitTradeData(order.market_id,true,false);
				addToOrder(order, value, total);
			});
		} else {
			fs.writeFileSync('./market_status/' + order.market_id + '.txt', 'true');
			isProccessing[order.user_id] = false;
	...
	if(arr.amount == order.amount) {
		...
		subtractWallet(order.user_id,order.market.coin_id_first,order.amount,order.coinFirst.symbol, function (err,balance) {
			...
			// process buyer
			plusWallet(arr.user_id,order.market.coin_id_first,order.amount,order.coinFirst.symbol, function () {

				// process seller
				var value = arr.value;
				var total = value - (value * order.fee / 100);

				plusWallet(order.user_id,order.market.coin_id_second, total, order.coinSecond.symbol, function () {

					socket.emit('notification', {channel: 'private-' + order.user_id, password: SOCKET_PASSWORD, message: {data: "Sold "+arr.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and plus "+total.toFixed(8)+" " + order.coinSecond.symbol, market_id: order.market_id}});
					socket.emit('notification', {channel: 'private-' + arr.user_id, password: SOCKET_PASSWORD, message: {data: "Bought "+order.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and minus "+arr.total+" " + order.coinSecond.symbol, market_id: order.market_id}});
		...
		subtractWallet(order.user_id,order.market.coin_id_first,order.amount,order.coinFirst.symbol, function (err,balance) {
			...
			plusWallet(arr.user_id,order.market.coin_id_first,order.amount,order.coinFirst.symbol, function () {
				...
				plusWallet(order.user_id, order.market.coin_id_second, total, order.coinSecond.symbol, function () {
					...
					socket.emit('notification', {channel: 'private-' + order.user_id, password: SOCKET_PASSWORD, message: {data: "Sold "+order.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and plus "+total.toFixed(8)+" " + order.coinSecond.symbol, market_id: order.market_id}});
					// proccess buyer
					...
					var totalBuyerPay = value + (value * order.fee / 100);
					socket.emit('notification', {channel: 'private-' + arr.user_id, password: SOCKET_PASSWORD, message: {data: "Bought "+order.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and minus "+totalBuyerPay.toFixed(8)+" " + order.coinSecond.symbol, market_id: order.market_id}});
	...
	if(arr.amount < order.amount) {
		...
		// process seller
		subtractWallet(order.user_id, order.market.coin_id_first, arr.amount, order.coinFirst.symbol, function (err, balance) {
			if(err) return callback();

			// process buyer
			plusWallet(arr.user_id,order.market.coin_id_first,arr.amount,order.coinFirst.symbol, function () {
				...
				plusWallet(order.user_id,order.market.coin_id_second,total,order.coinSecond.symbol, function () {
					socket.emit('notification', {channel: 'private-' + arr.user_id, password: SOCKET_PASSWORD, message: {data: "Bought "+arr.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and minus "+arr.total.toFixed(8)+" " + order.coinSecond.symbol, market_id: order.market_id}});
					// process seller
					socket.emit('notification', {channel: 'private-' + order.user_id, password: SOCKET_PASSWORD, message: {data: "Sold "+arr.amount+" "+order.coinFirst.symbol+" at "+arr.price+" "+order.coinSecond.symbol+" and plus "+total.toFixed(8)+" " + order.coinSecond.symbol, market_id: order.market_id}});
...
function setOrderStopLimit (data) {
	...
	subtractWallet(data.user_id,data.market.coin_id_first, data.amount, data.coinFirst.symbol, function(err, res) {
		...
		pool.query("INSERT INTO order_stop_limit(market_id,user_id,type,stop,price,amount,value,fee,total,created_at,updated_at) VALUES("+data.market_id+","+data.user_id+", 'sell' ,"+data.stop+","+data.price+", "+data.amount+", "+value+", "+data.fee+", "+total+",'"+getNowFormat()+"','"+getNowFormat()+"')", function () {
			// emit all my stop limit order in private
			pool.query("SELECT * FROM order_stop_limit WHERE market_id="+data.market_id+" AND user_id="+data.user_id+" ORDER BY id desc LIMIT 100", function(err, res) {
...
function getOrderAvaiable (data) {
	...
	if(data.type == 'limit') {
		var query = "SELECT * from orders WHERE type='buy' AND price >= "+data.price+" AND market_id="+data.market_id+" order by price desc, id";
	} else if (data.type == 'market') {
		var query = "SELECT * from orders WHERE type='buy' AND market_id="+data.market_id+" order by price desc, id";
	}
	...
	pool.query(query, function (err, res) {
		...
		if(res.rows.length == 0) {
			...
			if(data.type == 'limit') {
				subtractWallet(data.user_id, data.market.coin_id_first, data.amount, data.coinFirst.symbol, function (err,avaiable) {
					...
					var value = parseFloat(data.amount) * parseFloat(data.price);
					var total = value - (value * data.fee / 100);
...
function addToOrder (data, value, total) {
	...
	pool.query("INSERT INTO orders(market_id,user_id,type,price,amount,value,fee,total,created_at,updated_at) VALUES("+data.market_id+","+data.user_id+",'sell',"+data.price+","+data.amount+","+value+","+data.fee+","+total+",'"+getNowFormat()+"', '"+getNowFormat()+"')", function (err) {
		...
		emitTradeData(data.market_id, false, true);
